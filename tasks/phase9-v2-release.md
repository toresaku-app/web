...
### Track 3: iOS v1.2.0 提出（9/8〜12）→ **2026-09-20 提出済み・審査待ち**
...
- [x] シミュレータでの総点検（2026-09-19〜20）。選択→調整→PDF共有、チェック表ON/OFF、縦横、発行者、2枚イラストを確認。実機での印刷確認のみ残（下記）
...
- [x] ビルドした IPA で日本語ローカライズを確認（2026-09-20）。アーカイブ内 `app.app/ja.lproj` と `CFBundleDevelopmentRegion=ja` を確認済み
- [x] 規制対象の医療用デバイス: 「いずれの国または地域においても該当しない」と申告（2026-09-20）
- [x] 年齢レーティングの質問票を回答し直し（2026-09-20・利用者判断）。医療または治療に関する情報=まれ、健康またはウェルネス関連=はい → **4+ から 13+ に変更**。他項目はすべて「なし/いいえ」
- [x] 審査メモを追記（2026-09-20）。既存の SaMD 非該当・操作手順に、診断/自動推奨をしないこと・データは端末内のみ・アカウント不要を日英で追加
- [x] 提出（2026-09-20 21:00）。ビルド 1.2.0(1) を `xcrun altool` で API キー経由アップロード → 審査へ提出。ステータス「審査待ち」。リリースは承認後に自動
- [ ] 審査結果の対応（最大48時間・メール通知）
- [ ] 実機で PDF を1回出力・印刷し、A4 と余白を確認（DESIGN §7 の PROVEN_SAFE 確定用・利用者作業3分）
...
- [x] 発行者情報（施設名・担当者名）のPDF記載 — 実装済み。どちらも任意
  ...
  - [x] **プライバシーポリシーの公開反映**（2026-08-07 完了）← 継続保存は既存の記述と矛盾していたため
        別リポジトリ toresaku-app/privacy-policy に反映済み。公開ページで確認済み
        （最終更新日 8/6・「指導書データの保存について」に段落追加・改訂履歴に1行）
