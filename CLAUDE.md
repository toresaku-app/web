...
## 参照ドキュメント
- docs/decisions.md - 決定事項と却下した案（プロダクト・技術・事業・学会）。同じ議論を繰り返さないための記録
- docs/dev-notes.md - 開発・運用ノート（変更時に一緒に見るファイル、運動データ追加手順、環境固有の注意）
...
- 運動データは静的ファイル（exercises.ts + assets/illustrations/）で管理、外部DB不使用
- 運動の追加はアプリアップデートで対応（ユーザーによるカスタム運動作成は実装しない）
